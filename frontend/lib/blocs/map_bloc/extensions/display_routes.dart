  ...
  Future<void> _onDisplayAlternativeRoutes(DisplayAlternativeRoutes event, Emitter<MapState> emit,) async {
    try {
      ...
      for (int i = 0; i < routes!.length; i++) {
        ...
        await _addLine(coordinates, i);
        alternativeRoutes.add(r);
      }
      emit(state.copyWith(errorMessageGetter: () => null, alternativeRoutes: alternativeRoutes));
    } catch (e) {
      emit(state.copyWith(errorMessageGetter: () => 'Error displaying alternative routes: $e'));
    }
  }

  Future<void> _addLine(List<dynamic> fixedRoute, int i) async {
